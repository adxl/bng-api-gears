...
describe('Tests reports', () => {
  ...
  describe('Test find one report', () => {
    it('should return one report', async () => {
      const report = await reportsController.findOne({ id: '66666666-31bb-4b54-98f1-77610bc44b03' });
    ...
    it('should throws a not found exception', async () => {
      await expect(reportsController.findOne({ id: '66666666-31bb-4b54-98f1-77610bc44b53' })).rejects.toThrow();
    });
  });
  ...
  describe('Test create report', () => {
    it('should return an UUID', async () => {
      ...
      expect((await reportsController.create({ body })).identifiers[0].id).toHaveLength(36);
    });
  });
