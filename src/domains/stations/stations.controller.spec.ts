...
describe('Tests stations', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      imports: [TypeOrmModule.forRoot(TypeOrmConfig), TypeOrmModule.forFeature([Station]), StationsModule],
  ...
  describe('Test find one station', () => {
    it('should return one station', async () => {
      const station = await stationsController.findOne('11111111-bab3-439d-965d-0522568b0004');
    ...
    it('should throws a not found exception', async () => {
      await expect(stationsController.findOne('11111111-bab3-439d-965d-0522568b0222')).rejects.toThrow();
    });
  });
  ...
  describe('Test create station', () => {
    it('should return an UUID', async () => {
      ...
      expect((await stationsController.create(data)).identifiers[0].id).toHaveLength(36);
    });
  });
  ...
  describe('Test remove one station', () => {
    it('should return the number of affected resources', async () => {
      ...
      expect((await stationsController.remove(data)).affected).toEqual(1);
    });
  });
});
